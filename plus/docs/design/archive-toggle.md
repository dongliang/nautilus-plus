...
状态:**已实现**(扩展 `project-name-zh.py`,2026-08-22)

## 背景

分组视图(见 `grouped-view.md`)以 `archived: true` 为测试驱动,但此前只能手工编辑 `.project.yaml` 改归档状态。本功能在选中文件的右键菜单提供「归档」/「取消归档」,复用「修改中文名」已建立的 YAML 局部更新基础设施。

## 需求决策(用户已确认)

1. **支持多选批量**:选中多个文件夹时仍显示菜单项,批量处理
2. **单项动态标签**:全部已归档 → 「取消归档」;否则 → 「归档」。与全局开关的动作式标签风格一致
3. **无数据则删文件**:取消归档移除 `archived` 键后若文件再无任何数据,连同删除 `.project.yaml`(与清空中文名语义一致)
...
「修改中文名」原有的 `_replace_yaml_name` / `_remove_yaml_name` 泛化为按键名操作:
...
| `_remove_key_in_folder(folder, key)` | 中文名清除走同一摘键路径 |
...
get_file_items(files)
  ...
  ├─ 全部过滤掉 → 无菜单
  ├─ len(files)==1 → 「修改中文名」+ 归档切换项
  │   (多选即使过滤后只剩一个也不给「修改中文名」——按用户实际选择的数量判断)
...
- **归档**:缺失 `.project.yaml` 时创建只含 `archived: true` 的文件;已有文件按 marks 只替换值区域(`archived: false # 注释` → `archived: true # 注释`)
- **取消归档**:整行摘除(含行尾注释);flow 式 `{archived: true, name-zh: a}` → `{name-zh: a}`(摘条目+相邻逗号);摘后无数据删除整个文件
- malformed / 非 mapping / 重复键 / 超大文件:**报错不覆盖**,与中文名编辑同一防护
