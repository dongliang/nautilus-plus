...
- **Python 扩展**:改 `plus/extensions/python/project-name-zh/`;按下方验收流程**本地安装到系统**生效(sudo 不可用时临时装 `~/.local/share/nautilus-python/extensions/`,但系统副本必须删除,两者同存会双菜单——pitfalls #5);改后 `nautilus-plus -q` 重启生效;错误在 nautilus stderr
...
4. **验收内容**:`plus/dev-demo/` 内容与 `/tmp/opencode/demo` 一致——`alpha-project`(name-zh + archived)、`beta-project`(仅 name-zh)、`gamma-project`(仅 archived)、`plain-dir`(无 yaml)、`notes.txt`(普通文件);分别用图标视图和列表视图检查分组/中文名显示效果
